...
function evidenceOf(requestLine, requestHeaders, res) {
    return {
        requestLine,
        requestHeaders,
...
export const missingSecurityHeaderVerifier = {
    ...
    async verify(f, authHeaders) {
        const url = targetUrl(f);
        const hay = `${f.templateId ?? ""} ${f.name}`.toLowerCase();
        const headerName = Object.keys(HEADER_NAMES).find((name) => HEADER_NAMES[name].test(hay)) ?? "content-security-policy";
...
export const TODO_VERIFIERS = [];
export const VERIFIERS = [missingSecurityHeaderVerifier, ...TODO_VERIFIERS];
