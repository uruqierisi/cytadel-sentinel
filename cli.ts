...
program
  ...
  .action(async (scopeFile: string, opts: { allowDestructive: boolean; detach: boolean }) => {
    try {
      ...
      if (outcome.status === "COMPLETED") {
        ...
        if (outcome.engagementId) console.log(`  DefectDojo engagement: ${outcome.engagementId}`);
      } else {
        console.error(`\n✗ run FAILED: ${runId}\n  ${outcome.error}`);
        process.exitCode = 1;
      }
