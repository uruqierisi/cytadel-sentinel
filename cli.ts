  ...
  .action(async (file: string) => {
    try {
      ...
      if (err instanceof ScopeValidationError) {
        ...
        console.error("✗ scope validation error:", (err as Error).message);
      }
      process.exitCode = 1;
    } finally {
...
program
  ...
  .action(async (scopeFile: string, opts: { out?: string }) => {
    try {
      ...
      if (err instanceof ScopeValidationError) {
        ...
        console.error("✗ roe error:", (err as Error).message);
      }
      process.exitCode = 1;
    } finally {
      await disconnectDb().catch(() => undefined);
...
program
  ...
  .action(async (scopeFile: string, opts: { allowDestructive: boolean; detach: boolean; iUnderstandProduction: boolean }) => {
    try {
