...
import type { VerificationClass } from "./types.js";
...
export function classify(finding: UnifiedFinding): VerificationClass {
  const hay = `${finding.templateId ?? ""} ${finding.name} ${finding.sourceTool}`.toLowerCase();

  if (DESTRUCTIVE.test(hay)) return "NEVER_AUTO";
  if (MANUAL.test(hay)) return "MANUAL";
  if (AUTO_HINTS.some((re) => re.test(hay))) return "AUTO";
