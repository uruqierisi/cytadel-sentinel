...
export async function contextForJob(data: RunJobData): Promise<RunContext> {
  ...
  if (run.scopeHash !== loaded.scopeHash) {
    throw new Error(
      `scope file changed since the run was queued (hash ${run.scopeHash.slice(0, 12)} != ${loaded.scopeHash.slice(0, 12)}) — refusing to run`,
    );
  }
